from src.models import Elevator, ElevatorRequest, ElevatorSystem
...
def move_elevator(elevator_object: Elevator, elevator_system: ElevatorSystem):
    ...
    for elev_request in requests_pending:
        ...
        # 1
        if request_destination < 0 or request_destination > elevator_system.total_floors or request_start < 0 or request_start > elevator_system.total_floors:
...
def check_elevator_system(elevator_system: ElevatorSystem):
    ...
    for elevator in elevators_running:
        move_elevator(elevator_object=elevator,
                      elevator_system=elevator_system)
